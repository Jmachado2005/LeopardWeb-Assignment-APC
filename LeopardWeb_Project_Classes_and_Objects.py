...
class Student(User):
    ...
    def addDrop_course(self, choice, crnNum, studentID):
        ...
        print(f"AD_choice {choice}")
        AD_choice = choice

        if AD_choice == "Add":   # ADD
            ...
            if course is not None:   # if the course exists
                ...
                cursor.execute("""SELECT COURSE_ONE, COURSE_TWO, COURSE_THREE, COURSE_FOUR, COURSE_FIVE FROM STUDENT WHERE ID = ?""", (studentID,))
                ...
                crn_choice = int(crnNum)
                # DEBUGGING print(f"crn_choice here: {crn_choice}")
                ...
                if crn_choice not in schedule:
                    ...
                    return 3    # course does not exist in schedule
                else:
                    ...
                    cursor.execute(f"""UPDATE STUDENT SET {course_column} = NULL WHERE ID = ?""", (studentID,))
    ...
    def check_conflicts(self, studentID):
    ...
    def print_schedule(self, studentName):
        ...
        cursor.execute("""SELECT COURSE_ONE FROM STUDENT WHERE NAME = ?""", (studentName,))   # course 1 get CRN query
        ...
        cursor.execute("""SELECT COURSE_TWO FROM STUDENT WHERE NAME = ?""", (studentName,))
        ...
        cursor.execute("""SELECT COURSE_THREE FROM STUDENT WHERE NAME = ?""", (studentName,))
        ...
        cursor.execute("""SELECT COURSE_FOUR FROM STUDENT WHERE NAME = ?""", (studentName,))
        ...
        cursor.execute("""SELECT COURSE_FIVE FROM STUDENT WHERE NAME = ?""", (studentName,))
...
class Instructor(User):
    ...
    def search_student(self, student, course):
    ...
    def print_roster(self, courseCRN):
